...
import { Award, CheckCircle2, Coins, Home, IndianRupee, RefreshCw, ShieldCheck, Sparkles, TrendingUp } from 'lucide-react';
...
export const GoldRateSection: React.FC<GoldRateSectionProps> = ({ rates, rateData, isRefreshing, onRefresh }) => {
  ...
  const rateItems = useMemo(() => [
    { purity: '24K', title: '24K Pure Gold', fineness: '99.9% Pure (999)', rate: liveRates['24K'], icon: Coins, tag: 'Pure Bullion Standard', quality: '24K 999 Pure Bullion' },
    { purity: '22K', title: '22K Standard Hallmark', fineness: '91.6% BIS Hallmark (916)', rate: liveRates['22K'], icon: Award, tag: 'Most Traded · 916 Standard', quality: 'BIS 916 Hallmarked Gold', featured: true },
    { purity: '18K', title: '18K Fine Gold', fineness: '75.0% Fine (750)', rate: liveRates['18K'], icon: Sparkles, tag: 'Diamond / Studded Jewellery', quality: '18K 750 Fine Standard' },
  ], [liveRates]);
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-3 lg:gap-6">
          {rateItems.map((item) => {
            ...
            return <article key={item.purity} className={`gold-rate-card group relative overflow-hidden rounded-2xl p-5 sm:p-6 ${item.featured ? 'gold-rate-card-featured' : ''}`}>
              ...
              <div className="space-y-2"><div className="flex items-center justify-between rounded-full border border-[#D7B04A]/30 bg-[#FFF9E9] px-3 py-2 text-xs"><span className="text-[#756A5A]">Standard</span><strong className="flex items-center gap-1 text-[#3A2C1B]"><ShieldCheck className="h-3.5 w-3.5 text-[#C18A18]" />{item.quality}</strong></div><div className="flex items-center justify-between rounded-full border border-[#D7B04A]/30 bg-[#FFF9E9] px-3 py-2 text-xs"><span className="text-[#756A5A]">Testing</span><strong className="flex items-center gap-1 text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5" />German XRF (0% Melting Loss)</strong></div></div>
            </article>;
          })}
        </div>
